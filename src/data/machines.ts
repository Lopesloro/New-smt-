...
/**
 * MSTECH machine catalog — single source of truth.
 *
 * Field guide:
 *   - `slug`        url segment (kebab-case)
 *   - `name`        model code (mono in UI)
 *   - `fullName`    name + descriptor for headlines
 *   - `specs`       leave [] until datasheet arrives — empty state CTA fires
 *   - `media.video` add when filmed
 *   - `media.rotation360.frames` add when 360° photos arrive — UI tab auto-appears
 *   - `featured`    true → shown in /showroom and on the home page
 */

const defaultHotspots = (key: string, items: Array<{ x: number; y: number; title: string; text: string }>) =>
  items.map((i) => ({ ...i, _key: `${key}` }));
...
export const machines: Machine[] = [
  ...
  },
  {
    ...
    description:
      "Integrated or stand-alone vision module for fiducial recognition, edge detection and cutting inspection. Available across most MSTECH router and sawing systems.",
  ...
  },
  {
    ...
    description:
      "Custom factory automation engineered for automotive assembly lines. MSTECH's FA division designs end-to-end automation for body, ECU and component lines.",
...
// Silence unused helper export
void defaultHotspots;
...
export const getRelatedMachines = (m: Machine, limit = 3) =>
  machines
    .filter(
      (x) =>
        x.slug !== m.slug && x.category === m.category && x.subcategory === m.subcategory,
    )
    .slice(0, limit);
