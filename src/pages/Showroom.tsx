...
export default function Showroom() {
  ...
  const machine = featured[idx];

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowRight") setIdx((i) => (i + 1) % featured.length);
      if (e.key === "ArrowLeft") setIdx((i) => (i - 1 + featured.length) % featured.length);
    };
  ...
  return (
    <>
      <Helmet>
        <title>Showroom · MSTECH machines · SMTS</title>
        <meta
          name="description"
          content="Interactive showroom — explore featured MSTECH depaneling machines with hotspots, video walkthroughs and 360° rotation."
        />
      </Helmet>
        ...
        <div className="mx-auto flex max-w-[1280px] items-center justify-between gap-4 px-6 py-4 lg:px-12">
          <div>
            ...
            <h1 className="mt-1 font-display text-xl uppercase tracking-tight md:text-2xl">
              Explore featured machines
            </h1>
          </div>
          <div className="hidden gap-2 font-mono-tech text-xs uppercase tracking-widest text-white/55 md:flex">
        ...
        <div className="mx-auto max-w-[1280px] px-6 py-10 lg:px-12 lg:py-16">
          <div className="grid gap-10 lg:grid-cols-[1.5fr,1fr] lg:gap-14">
            <div className="relative">
              ...
              <div className="mt-6 flex items-center justify-between">
                <button
                  type="button"
                  onClick={prev}
                  className="inline-flex items-center gap-2 rounded-md border border-white/15 px-4 py-2 font-mono-tech text-xs uppercase tracking-widest text-white/75 hover:border-[var(--brand-lime)] hover:text-white transition"
            ...
            <div>
              ...
              <p className="mt-2 text-base text-white/75">{machine.fullName}</p>
              <p className="mt-5 text-base leading-relaxed text-white/85">{machine.tagline}</p>
              <p className="mt-3 text-sm leading-relaxed text-white/65">{machine.description}</p>
              ...
              {cat && (
                <Link
                  ...
                  className="mt-8 inline-flex items-center gap-2 rounded-md bg-[var(--brand-lime)] px-5 py-2.5 font-display text-xs uppercase tracking-widest text-[var(--brand-green-dark)] hover:bg-[var(--brand-lime-bright)] transition"
                >
      ...
      <section>
        <div className="mx-auto max-w-[1280px] px-6 py-10 lg:px-12 lg:py-14">
          <p className="font-mono-tech text-xs uppercase tracking-widest text-white/55">
            Featured machines
          </p>
          <div className="mt-6 grid gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7">
            {featured.map((m, i) => {
              const active = i === idx;
              return (
                <button
                ...
                >
                  ...
                  <div className="p-2.5">
                    <p className="font-display text-xs uppercase tracking-wide text-white">{m.name}</p>
                  </div>
                </button>
              );
            })}
